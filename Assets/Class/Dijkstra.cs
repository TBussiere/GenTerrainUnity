...
namespace Assets.Class
{
    ...
    sealed class Graph
    {
        ...
        public Graph(int vertexCount) => adjacency = Enumerable.Range(0, vertexCount).Select(v => new EdgeList()).ToList();
        ...
        {
            var info = Enumerable.Range(0, adjacency.Count).Select(i => (distance: double.PositiveInfinity, prev: i)).ToArray();
